...
import React, { useState, Suspense } from 'react'; // 1. Added Suspense
import { useSearchParams, useRouter } from 'next/navigation';
import { Lock, KeyRound, CheckCircle2, ShieldCheck } from 'lucide-react';
import axios from 'axios';

// 2. Created a sub-component to handle the search params logic safely
function RegistrationFormContent() {
  ...
  const handleFinalizeAccount = async (e: React.FormEvent) => {
    ...
    try {
      await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/api/Users/complete-registration`, {
        token: token,
        tenantId: tenantId,
        password: password
      });
        ...
        <form onSubmit={handleFinalizeAccount} className="space-y-4">
          ...
          <div>
            ...
            <div className="relative mt-2">
              ...
              <input type="password" required minLength={6} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••" className="block w-full rounded-lg border border-slate-800 bg-slate-950 py-2.5 pl-10 pr-3 text-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500" />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="relative mt-2">
              ...
              <input type="password" required value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} placeholder="••••••••" className="block w-full rounded-lg border border-slate-800 bg-slate-950 py-2.5 pl-10 pr-3 text-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500" />
            </div>
          </div>

          <button type="submit" disabled={loading} className="w-full rounded-lg bg-indigo-600 py-2.5 text-sm font-semibold text-white transition-all hover:bg-indigo-500 disabled:opacity-50">
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-slate-950 p-4 text-white font-sans antialiased">
      <div className="w-full max-w-md space-y-6 rounded-xl border border-slate-800 bg-slate-900/50 p-8 backdrop-blur-sm">
        <Suspense fallback={<div className="text-center text-slate-400">Loading security protocols...</div>}>
